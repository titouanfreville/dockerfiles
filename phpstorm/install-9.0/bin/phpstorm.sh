...
for vm_opts_file in "$IDE_BIN_HOME/phpstorm$BITS.vmoptions" "$HOME/.WebIde90/phpstorm$BITS.vmoptions" "$WEBIDE_VM_OPTIONS"; do
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=WebIde90"
  fi
fi
